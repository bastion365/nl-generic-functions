...
await test('unauthoritative', async t => {
    ...
    await t.test('created in admin directory', async () => {
        adminDirectoryRequestBundle = JSON.parse(
            (await fsp.readFile('data/admin-directory/unauthoritative.json', 'utf-8')).replace(/{SALT}/g, salt),
        )
    ...
    await t.test('created in LRZa mock (without endpoint)', async () => {
        const bundle = JSON.parse((await fsp.readFile('data/lrza-mock/unauthoritative.json', 'utf-8')).replace(/{SALT}/g, salt))
    ...
    await t.test('Organization cannot be found in query directory by name', async () => {
        const { entry = [] } = await (
    ...
    await t.test('Location cannot be found in query directory by name', async () => {
        const { entry = [] } = await (
    ...
    await t.test('HealthcareService cannot be found in query directory by name', async () => {
        const { entry = [] } = await (
    ...
    await t.test('Endpoint cannot be found in query directory by name', async () => {
        const { entry = [] } = await (
